...
##from . import _brorest
from .._core.gwseries import GwSeries

_logger = _logging.getLogger(__name__)


def get_gwseries_from_bro_gmw(gmwid=None, tube=None):
    """Return GwSeries object with levels from BRO REST service.
        ...
        """
    bgw = BroGwSeries.from_server(gmwid=gmwid, tube=tube)
    return bgw.gwseries
...
class BroGwSeries:
    ...
    def from_server(cls, gmwid=None, tube=None):
        ...
        if gmw._tree is None:
            _logger.warning((f"Invalid XML tree for {gmwid} tube {tube}."))
        ...
        tubeprops, wellprops, wellevents = cls._get_gmw_properties(gmw, tube)
        if tubeprops.empty:
            # Sometimes, the server returns a GMWID for which no data
            # can be retrieved. Mysterious...
            return cls(wellprops=Series(), wellevents=DataFrame(), 
                tubeprops=Series(), gldprops=DataFrame(), obsprops=DataFrame(), 
                obs=DataFrame(), procesprops=DataFrame(), 
                timeseriescounts=DataFrame(), gmwid=gmwid, tube=tube)
        ...
        tube = str(tube)
        brest = BroREST()
        ##tubegld = brest.get_gmwtubes(gmwid)
        tubegld = brest.get_gld_from_gmw(gmwid)
        ...
        for gldid in tubegld['gldid'].values:
            ...
            if gld._tree is not None:
                ...
                _logger.warning((f"Invalid GLDXML for {gldid} of {gmwid}."))
        ...
        if gldxml: 
            ...
            _logger.warning((f'No valid GLD found for {gmwid} tube {tube}.'))
    ...
    def _get_gmw_properties(cls, gmw, tube):
        ...
        if gmw.tubeprops.empty:
            # Sometimes, the BRO server returns a GMWID for which no 
            # actual data can be retrieved...
            tubeprops=DataFrame()
            wellprops=DataFrame()
            wellevents=DataFrame()
    ...
    def gwseries(self):
        ...
        if pd.isnull(self._wellprops.loc['wellConstructionDate']):
            startdate = self.heads.index.min() # timestamp
            _logger.warning((f'Tube {self.tube} of {self.gmwid} has no '
