...
def gmw_exists(gmwid):
    ...
    try:
        ...
        # gmw does exist, but it has no gld content
        # example 'GMW000000037515'
        err_type, err, err_traceback = _sys.exc_info()
        if str(err)==f"name 'gmw' is not defined":
            return False
        elif str(err)==f"No gmw found":
            return False
        else:
            raise
    except Exception: #as err:
        err_type, err, err_traceback = _sys.exc_info()
        # if error mesage shows a non existent gldid was
        # requested, return None. Else reraise exception.
        errmsg = f'Retrieving data from https://publiek.broservices.nl/gm/gmw/v1/objects/{gmwid} failed'
        if str(err)==errmsg:
            return False
        else:
            raise
...
def get_gmwprops(gmwid):
        ...
        propskeys = [
            ('nitgcode', 'nitgCode'),
            ('wellcode', 'wellCode'),
            ('tubecount', 'numberOfMonitoringTubes'),
            ('ownerid', 'owner'),
            ('observerid', 'deliveryAccountableParty'),
            ('gridreference', 'coordinateTransformation'),
            ('registration', 'registrationStatus'),
            ('surface', 'groundLevelPosition'),
            ('surfacemethod', 'groundLevelPositioningMethod'),
            ('surfacestable', 'groundLevelStable'),
            ('mysterious_id', 'id'),
            ('heightdatum', 'verticalDatum'),
            ('wellconstructiondate', 'wellConstructionDate'),
            ('wellprotection', 'wellHeadProtector'),
            ('deregistered', 'deregistered'),
            ('deregistrationtime', 'deregistrationTime'),
            ]
        for (key1, key2) in propskeys:
            if key1 in gmwdict.keys():
                gmwprops[key1] = gmwdict[key2]
            else:
                gmwprops[key1] = _np.nan
        ...
        """
        if 'groundLevelPosition' in gmwdict.keys():
            if gmwdict['groundLevelPosition'] is not None:
                gmwprops['surface'] = float(gmwdict['groundLevelPosition'])
            else:
                gmwprops['surface'] = _np.nan

        if not _pd.isnull(gmwprops['wellconstructiondate']):
            gmwprops['wellconstructiondate'] = gmwprops['wellconstructiondate'].strftime('%d-%m-%Y')
...
def get_tubeprops(gmwid, tube):
    ...
    if 'monitoringTube' in gmwdict.keys():
    ...
    tubekeys = [
        ('startdate', 'wellConstructionDate'),
        ('surfacelevel', 'groundLevelPosition'),
        ]
    for (key1, key2) in tubekeys:
        if key2 in gmwdict.keys():
            tp[key1] = gmwdict[key2]
        else:
            tp[key1] = _np.nan
    #tp['startdate'] = gmwdict['wellConstructionDate']
...
def get_tubeobs_from_gmw(gmwid):
    ...
    if not gmw_exists(gmwid):
        ...
        _logger.warning(f'Invalid gmwid {gmwid}.')  
...
def get_tubeobs(gmwid, tube):
    ...
    if obs.empty: # yes, wells without observations do exist.
        ...
        _logger.warning(f'{gmwid}: No observations for tube {tube}.')
    else:
